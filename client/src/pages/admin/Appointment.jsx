...
export default function AdminAppointments() {
  ...
  function forward(id) {
    setAppts(prev => prev.map(a => a.id === id ? { ...a, status: 'dikonfirmasi' } : a));
  }
  function tolak(id, nama) {
    if (window.confirm('Tolak appointment ' + nama + '?')) setAppts(prev => prev.filter(a => a.id !== id));
  }

  const statusLabel = { menunggu: 'Menunggu', dikonfirmasi: 'Dikonfirmasi Dokter', selesai: 'Selesai' };
      ...
      <div className="main-content">
        ...
        <div className="content-area">
          <div className="filter-row">
            {['semua', 'menunggu', 'dikonfirmasi', 'selesai'].map(f => (
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {filtered.map(a => (
                    <tr key={a.id}>
                      <td><div style={{ fontWeight: 600 }}>{a.pasien}</div><div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{a.keluhan}</div></td>
                      <td><div style={{ fontWeight: 600 }}>{a.dokter}</div><div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{a.spesialis}</div></td>
                      ...
                      <td>
                        {a.status === 'menunggu' ? (<>
                          ...
                          <button className="btn-action btn-tolak" style={{ marginLeft: 6 }} onClick={() => tolak(a.id, a.pasien)}>Tolak</button>
                        </>) : (
        ...
        <div className="modal-overlay open" onClick={() => setDetail(null)}>
          <div style={{ background: 'white', borderRadius: 20, padding: 28, width: '100%', maxWidth: 520, boxShadow: '0 20px 60px rgba(0,0,0,0.25)', animation: 'slideUp 0.3s ease' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ background: 'linear-gradient(135deg,#0D1B4B,#1a2a6c)', borderRadius: 14, padding: '20px 24px', marginBottom: 18, color: 'white', display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, fontWeight: 800 }}>{detail.pasien.charAt(0)}</div>
              <div><div style={{ fontSize: 20, fontWeight: 800 }}>{detail.pasien}</div><div style={{ fontSize: 13, opacity: 0.8, marginTop: 3 }}>{detail.dokter} · {detail.spesialis}</div></div>
            </div>
